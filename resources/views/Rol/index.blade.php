...
<div class="sticky-top px-3 mt-2">
    <div class="row bg-light">
        <div class="col">
            <p class="float-left text-secondary text-uppercase font-inter-700" style="font-size:13px;">{{'Lista de Roles'}}</p>
        </div>
        <div class="col">
...
<div class="container shadow rounded border border-3 h-90 bg-white">
        ...
        <div class="table-responsive">
            <table id="productsTable" class="table table-hover table-sm text-left mb-0" style="color:#6E6893 !important;">
                ...
                <tbody>
                    ...
                    <tr>
                        <th class="align-middle" scope="row">#{{$rol->RolId}}</th>
                        <td class="align-middle" scope="col">
                            <div class="text-nowrap">
                                <div class="text-dark">{{$rol->RolNombre}}</div>
                            </div>
                        </td>
                        <td class="align-middle" scope="col">
                            <div class="text-nowrap">
                                Fecha : {{$rol->updated_at}}
                                <br>
                        ...
                        <td class="align-middle text-right" scope="col">
                            <a href="{{route('permisos.edit', ['permiso' => $rol->RolId])}}" class="btn btn-sm btn-warning">
                                <div class="text-nowrap">Editar</div>
                            </a>
                        </td>
                        <td class="align-middle text-right" scope="col">
                            <form method="POST" id="formDestroy{{$rol->RolId}}" action="{{route('permisos.destroy', ['permiso' => $rol->RolId])}}">
                                @csrf
                                @method('DELETE')
    ...
    <div class="row flex-row d-flex" style="background-color: #F4F2FF; color:#6E6893 !important;">
        <div class="col my-auto">
            <div class="text-left">filas por página: {{$roles->count()}}</div>
        </div>

        <div class="col">
            <div class="text-right">
                <div class="mx-3">{{$roles->firstItem()}}-{{$roles->lastItem()}} of {{$roles->total()}}</div>
                <a href="{{$roles->url(1)}}"><i class="px-0 fas fa-angle-double-left"></i></a>
                <a href="{{$roles->previousPageUrl()}}"><i class="px-1 fas fa-chevron-left"></i></a>
                <a href="{{$roles->previousPageUrl()}}"><i class="px-1">{{$roles->currentPage() > 1 ? $roles->currentPage() - 1 : ''}}</i></a>
                <i class="px-0">{{$roles->currentPage()}}</i>
                <a href="{{$roles->nextPageUrl()}}"><i class="px-1">{{$roles->currentPage() + 1}}</i></a>
                <a href="{{$roles->nextPageUrl()}}"><i class="px-1 fas fa-chevron-right"></i></a>
                <a href="{{$roles->url($roles->lastPage())}}"><i class="px-0 fas fa-angle-double-right"></i></a>
            </div>
        </div>
    </div>
</div>
